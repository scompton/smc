...
##############################################################################
pngDir = "/Users/scompton/Pictures/simwSlides/"
#pngDir=None
...
def main(args):
  ...
  #go3S(200) # with initial compression

def makeSyntheticTraces():
  init(1.0)
  ptw = 504.0
  def plot(fA,s,vl,pngName):
    plot1(fA,s=s,vLabel=vl,vInterval=1.0,hLabel=None,hLimits=[-2.0,2.0],
          width=432,height=1096,o=x1dx2r,png=pngDir+pngName,paint=paintSlide)
...
def makeIndividual(dg1,dg2,dgS):
  for nrms in [0.0,1.0]:
    ...
    def plot(dw,e,u,ku,hLabel,c,pngName):
      se = dw.getSampling1()
      su = dw.getSamplingU()
      el = [se.getFirst(),se.getLast()]
      ul = [su.getFirst(),su.getLast()]
      ...
      WarpUtils.normalizeErrors(e)
      ne = se.getCount()
      uA = [[copy(ne,u),"u",c+"-",2.0],[copy(ne,ku),"ku","w--",2.0]]
      plot2(e,pA=uA,s1=se,s2=su,hLabel=hLabel,vLabel="Time shift (s)",
            cbar=None,clips1=[0,0.3],width=750,height=400,hLimits=el,vLimits=ul,
            o=x1rx2u,png=pngDir+pngName,paint=paintSlide)
    #plot(dw1,e1,u1c,ku1c, "PP time (s)","r","ae_u1"+label)
    #plot(dw2,e2,u2c,ku2c, "PP time (s)","b","ae_u2"+label)
    #plot(dwS,eS,uSg, kuS,"PS1 time (s)","g","ae_uS"+label)
    def plotU(u,ku,c,pngName,w=1000,h=450,vLimits=[-0.15,0.2],vFormat=None):
    ...
    def plotGS(u1,u2,method,c,pngName):
      ...
      plot1([[gs,c+"-"],[kgs,"k--"]],s=sf,lineWidth=2.0,hLabel="PP time (s)",
            ...
            png=pngDir+pngName,paint=paintSlide)
    ...
    plot1(gsdA,s=sf,lineWidth=4.0,hLabel="PP time (s)",vLabel="Residual",
          vLimits=[-0.1,0.1],vFormat="%10f",width=1000,height=435,o=x1rx2u,
          bgColor=bgColor,png=pngDir+"simw_gsr_"+label,paint=paintSlide)
...
def makeSimultaneous(dg1):
  for nrms in [0.0,1.0]:
    ...
    plot1(gsdA,s=sf,lineWidth=4.0,hLabel="PP time (s)",vLabel="Residual",
          vLimits=[-0.1,0.1],vFormat="%10f",width=1000,height=435,o=x1rx2u,
          bgColor=bgColor,png=pngDir+"simw_gsrSim_"+label,paint=paintSlide)
    s1 = dw.getSampling1()
    n1 = s1.getCount()
    plotError3(e,dw,copy(n1,u1c),copy(n1,uSPPc), True,"simw_e3d_"+label)
    plotError3(e,dw,copy(n1,u1c),copy(n1,uSPPc),False,"simw_e3dNoU_"+label)

def init(nrms):
  global sg,sh,f,g,h,ku1,ku2,kuS,kvpvs1,kvpvs2,kgs
  f,g,h,sg,sh,ku1,ku2,kuS,kvpvs1,kvpvs2,kgs = makeSequences(nrms)

# Computes shifts for 3 images simultaneously
def go3S(dg1):
  print 30*"*"+" go3S "+30*"*"
  ext = "3S"
  e,u1,uSPP,dw,sgc,gc,shc,hc = warp3Compress(dg1)
  c = dw.getCompression()
  u1 = WarpUtils.getScaledShifts(sf,u1,c)
  uSPP = mul(c,uSPP) # stretch
  #uSPP = WarpUtils.interpUS(sf,u1,uSPP)
  u2 = add(u1,uSPP)
  w1 = WarpUtils.applyShifts(sf,u1,sg,g)
  w2 = WarpUtils.applyShifts(sf,u2,sh,h)
  se = dw.getSampling1()
  ne = se.getCount()
  print "NRMS  1: %g"%WarpUtils.computeNrms(ne,f,w1)
  print "NRMS  2: %g"%WarpUtils.computeNrms(ne,f,w2)
  #def plotShift(u,ku,title):
  #  plot1([[u,"k-"],[ku,"r--"]],s=sf,title=title,hLabel="PP time (s)",
  #        vLabel="Time shift (s)",vLimits=None,width=800,height=500,o=x1rx2u)
  #plotShift(WarpUtils.getScaledShifts(sf,u1,c),ku1,"u1 stretched "+ext)
  #plotShift(WarpUtils.getScaledShifts(sf,u2,c),ku2,"u2 stretched "+ext)
  #plotShift(u1,WarpUtils.getScaledShifts(sf,ku1,1.0/c),"u1 "+ext)
  #plotShift(u2,WarpUtils.getScaledShifts(sf,ku2,1.0/c),"u2 "+ext)
  #kuSPP = WarpUtils.ps1ToPpTime(sf,ku1,sg,kuS)
  #kuSPPc = mul(1.0/c,kuSPP) # compress
  #plotShift(uSPP,kuSPPc,"uSPP "+ext)
  #uSPPs = mul(c,uSPP) # stretch
  #plotShift(uSPPs,kuSPP,"uS stretched "+ext)
  #uSPPi = WarpUtils.interpUS(sf,u1,uSPPs)
  #plotShift(uSPPi,uSPPs,"uSPP vs uSPPi "+ext)
  if pw:
    plotWarped([[f,"k-"],[w1,"r-"]],sf,"PP","1 "+ext)
    plotWarped([[f,"k-"],[w2,"b-"]],sf,"PP","2 "+ext)
  plotVpVs(u1,kvpvs1,dw,sf,"PP","Vp/Vs 1 "+ext)
  plotVpVs(u2,kvpvs2,dw,sf,"PP","Vp/Vs 2 "+ext)
  plotGammaS(u1,u2,sf,WarpUtils.gammaSu12,"PP","GammaS u12 "+ext)
  plotGammaS(u1,uSPP,sf,WarpUtils.gammaSu1S,"PP","GammaS u1S "+ext)
  plotGammaS(u2,uSPP,sf,WarpUtils.gammaSu2S,"PP","GammaS u2S "+ext)
  #checkShifts(u1,u2,WarpUtils.getScaledShifts(sf,ku2,1.0/c),uSPP,sf,ext)
  #plotError3(e,dw,u1,uSPP)
  return u1,u2,uSPP
...
def plotError3(e,dw,u1,uS,plotU,pngName):
  se = dw.getSampling1()
